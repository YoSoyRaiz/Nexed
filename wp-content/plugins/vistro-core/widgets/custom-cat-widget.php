...
if ( class_exists( 'CSF' ) ) {
    ...
    if ( !function_exists( 'custom_cat_widget' ) ) {
        function custom_cat_widget( $args, $instance ) {
            ...
            $html .= '<div class="tx-cat-widget sidebar-category">';
                $html .= '<ul class="tx-cat-list list-unstyled">';
                foreach ( $instance['select_cats'] as $cat ) {
                    $html .= '<li class="tx-cat-item">';
                    $html .= '<a href="' . get_category_link( $cat['select_cat'] ) . '">';
                    $html .= '<span class="tx-icon">'.get_cat_name( $cat['select_cat'] ).'</span>';
                    $count = get_category($cat['select_cat'])->count;
                    $countFormatted = ($count < 10) ? '0' . $count : $count;
                    $html .= '<span class="post-count">(' . $countFormatted . ')</span>';
                    $html .= '</a>';
                    $html .= '</li>';
                }
                $html .= '</ul>';
