...
<div class="wrap about-wrap xriver-wrap">
    <h1><?php _e( 'Welcome to '.tf_theme_name().'', VISTRO_CORE_TEXT_DOMAIN );?></h1>

    <div class="about-text"><?php echo esc_html__( ''.tf_theme_name().' theme is now installed and ready to use!', VISTRO_CORE_TEXT_DOMAIN ); ?></div>
<div class="xriver-badge">
    <p><?php echo esc_html( $tatheme->get( 'Version' ) ); ?></p>
</div>
    <h2 class="nav-tab-wrapper">
        <?php
			printf( '<a href="#" class="nav-tab nav-tab-active">%s</a>', __( 'Welcome', VISTRO_CORE_TEXT_DOMAIN ) );
			printf( '<a href="%s" class="nav-tab">%s</a>', admin_url( 'admin.php?page=vistro-theme-option' ), __( 'Theme Options', VISTRO_CORE_TEXT_DOMAIN ) );
			printf( '<a href="%s" class="nav-tab">%s</a>', admin_url( 'admin.php?page=tf-demo-importer' ), __( 'Demo Import', VISTRO_CORE_TEXT_DOMAIN ) );
			printf( '<a href="%s" class="nav-tab">%s</a>', admin_url( 'admin.php?page=tf-documentations' ), __( 'Documentations', VISTRO_CORE_TEXT_DOMAIN ) );
		?>
    </h2>
    ...
    <div class="xriver-section nav-tab-active" id="welcome">
        <p class="about-description">
            <?php printf( __( 'Prior to commencing, we kindly request you to thoroughly consult the documentation, which is included in the theme folder or accessible from our <a href="https://themexriver.com/wp/'.strtolower(tf_theme_name()).'/'.strtolower(tf_theme_name()).'-doc" target="_blank">Website</a>. This comprehensive resource provides a wealth of valuable information and detailed instructions for utilizing '.tf_theme_name().'.', VISTRO_CORE_TEXT_DOMAIN ) );?>
        </p>
        <p class="about-description">
            <?php printf( __( 'In the event that you are unable to locate the desired information within our documentation, we encourage you to reach out to us via <a href="https://themexriver.ticksy.com/" target="_blank">Support Ticket</a> including your purchase code, site CPanel credentials, and admin login information.', VISTRO_CORE_TEXT_DOMAIN ), 'https://themexriver.ticksy.com/' );?>
        </p>
        <p class="about-description">
            <?php printf( __( 'Rest assured, we are delighted to assist you and anticipate providing you with a prompt response exceeding your expectations.', VISTRO_CORE_TEXT_DOMAIN ), 'https://themexriver.com/wp/'.strtolower(tf_theme_name()).'/'.strtolower(tf_theme_name()).'-doc' );?>
        </p>

        <p class="about-description">
		<mark class="error"><?php printf( __( 'Note: Please Install All Required Plugins Before Install Demo !', VISTRO_CORE_TEXT_DOMAIN ), 'https://themexriver.com/wp/'.strtolower(tf_theme_name()).'/'.strtolower(tf_theme_name()).'-doc' );?>
			</mark>
        </p>
    </div>
    ...
    <table class="system-status-table">
        <tbody>
                     <tr>
						<td><?php esc_html_e( 'WP Version', VISTRO_CORE_TEXT_DOMAIN );?></td>
						<td>
						...
						<tr>
							<td><?php esc_html_e( 'Language', VISTRO_CORE_TEXT_DOMAIN );?></td>
							<td><?php echo get_locale() ?></td>
						</tr>

						<tr>
							<td><?php esc_html_e( 'WP Memory Limit', VISTRO_CORE_TEXT_DOMAIN );?></td>
							<td>
								<?php
									$memory = xriver_let_to_num( WP_MEMORY_LIMIT );
									if ( $memory < 100663296 ) {
										echo '<mark class="error">' . sprintf( esc_html__( '%s - We recommend setting memory to at least 96MB. %s.', VISTRO_CORE_TEXT_DOMAIN ), size_format( $memory ), '<a href="' . esc_url( '//www.wpbeginner.com/wp-tutorials/fix-wordpress-memory-exhausted-error-increase-php-memory/' ) . '" target="_blank">' . esc_html__( 'More info', VISTRO_CORE_TEXT_DOMAIN ) . '</a>' ) . '</mark>';
									} else {
						...
						<tr>
							<td><?php esc_html_e( 'PHP Max Input Vars', VISTRO_CORE_TEXT_DOMAIN );?></td>
							<td>
								<?php
									$max_input = ini_get( 'max_input_vars' );
									if ( $max_input < 3000 ) {
										echo '<mark class="error">' . sprintf( wp_kses( __( '%s - We recommend setting PHP max_input_vars to at least 3000. See: <a href="%s" target="_blank">Increasing the PHP max vars limit</a>', VISTRO_CORE_TEXT_DOMAIN ), [ 'a' => [ 'href' => [], 'target' => [] ] ] ), $max_input, '//teconce.com/support/2018/12/05/increasing-max-input-vars/' ) . '</mark>';
									} else {
						...
						<tr>
						  <td>
						     <?php esc_html_e( 'PHP Version', VISTRO_CORE_TEXT_DOMAIN );?>
						  </td>
						...
						<tr>
						    <td>
						     <?php esc_html_e( 'Server Info', VISTRO_CORE_TEXT_DOMAIN );?>
						  </td>
						...
						<tr>
						    <td>
						        <?php esc_html_e( 'Secure Connection(HTTPS)', VISTRO_CORE_TEXT_DOMAIN );?>
						    </td>
						    <td>
						        <?php
    ...
    <table class="system-status-table">
        <tbody>
            <tr>
                <td><?php esc_html_e( 'Theme Name', VISTRO_CORE_TEXT_DOMAIN );?></td>
                <td><?php echo wp_get_theme(); ?></td>
            </tr>

             <tr>
                <td><?php esc_html_e( 'Author Name', VISTRO_CORE_TEXT_DOMAIN );?></td>
                <td><?php echo esc_html( $tatheme->get( 'Author' ) ); ?></td>
            </tr>

            <tr>
					<td><?php esc_html_e( 'Current Version', VISTRO_CORE_TEXT_DOMAIN );?></td>
					<td><?php echo esc_html( $tatheme->get( 'Version' ) ); ?></td>
				</tr>

				  <tr>
					<td><?php esc_html_e( 'Text Domain', VISTRO_CORE_TEXT_DOMAIN );?></td>
					<td><?php echo esc_html( $tatheme->get( 'TextDomain' ) ); ?></td>
				</tr>

				<tr>
				    <td><?php esc_html_e( 'Child Theme', VISTRO_CORE_TEXT_DOMAIN );?></td>
