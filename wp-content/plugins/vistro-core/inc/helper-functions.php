...
function elh_element_get_allowed_html_desc( $level = 'basic' ) {
    ...
    return sprintf( __( 'This input field has support for the following HTML tags: %1$s', 'vistro-core' ), '<code>' . esc_html( $tags_str ) . '</code>' );
}
...
function element_pack_position() {
    ...
    $position_options = [
        ''              => esc_html__( 'Default', 'vistro-core' ),
        'top-left'      => esc_html__( 'Top Left', 'vistro-core' ),
        'top-center'    => esc_html__( 'Top Center', 'vistro-core' ),
        'top-right'     => esc_html__( 'Top Right', 'vistro-core' ),
        'center'        => esc_html__( 'Center', 'vistro-core' ),
        'center-left'   => esc_html__( 'Center Left', 'vistro-core' ),
        'center-right'  => esc_html__( 'Center Right', 'vistro-core' ),
        'bottom-left'   => esc_html__( 'Bottom Left', 'vistro-core' ),
        'bottom-center' => esc_html__( 'Bottom Center', 'vistro-core' ),
        'bottom-right'  => esc_html__( 'Bottom Right', 'vistro-core' ),
    ];
...
// newsletter function
function ta_newsletter_popup() {

    $newsletter_popup_switch = get_theme_mod( 'newsletter_popup_switch', false );
    $newslater_image = get_theme_mod( 'newslater_image', get_template_directory_uri() . '/assets/bg/newsletter.jpg' );
    $newslater_title = get_theme_mod( 'newslater_title', __( 'Subscribe our newsletter', VISTRO_CORE_TEXT_DOMAIN ) );
    $newslater_description = get_theme_mod( 'newslater_description', __( 'Sign up to receive updates, promotions, and sneak peaks of upcoming products.', VISTRO_CORE_TEXT_DOMAIN ) );
    $newslater_shortcode = get_theme_mod( 'newslater_shortcode' );

    if ( $newsletter_popup_switch == true ) {
        $html = '
            <div class="tf-newsletter-popup-area">
                <div class="tf-newsletter-popup">
                    <button class="close-button" data-newsletter-close><i class="fal fa-times"></i></button>
                    <div class="thumb">
                        <img src="' . esc_url( $newslater_image ) . '" alt="">
                    </div>
                    <div class="content">
                        <div class="inner">
                            <div class="icon">
                            ...
                            <h2 class="title tf-title-uppercase">' . esc_html( $newslater_title ) . '</h2>
                            <p>' . esc_html( $newslater_description ) . '</p>
                            <div class="tf-form-field">
                                ' . do_shortcode( $newslater_shortcode ) . '
                            </div>
                        </div>
                    </div>
                </div>
...
// cookies
function ta_cookies_popup() {
    $cookies_popup_switch = get_theme_mod( 'cookies_popup_switch', false );
    $cookies_description = get_theme_mod( 'cookies_description', __( 'By browsing this website, you consent to our Privacy Policy.', VISTRO_CORE_TEXT_DOMAIN ) );
    $cookies_accept_btn = get_theme_mod( 'cookies_accept_btn', __( 'Accept', VISTRO_CORE_TEXT_DOMAIN ) );
    $cookies_decline_btn = get_theme_mod( 'cookies_decline_btn', __( 'Decline', VISTRO_CORE_TEXT_DOMAIN ) );

    if ( $cookies_popup_switch == true ) {
        $html = '
            <div class="tf-cookies" data-tf-cookies>
                <div class="icon mb-15">
                    <img src="' . get_template_directory_uri() . '/assets/img/icons/cookies-icon.webp ' . '" alt="img">
                </div>
                <p>' . ta_kses_basic( $cookies_description ) . '</p>
                <div>
                    <button class="cookie-btn accept" data-tf-cookies-btn>' . esc_html( $cookies_accept_btn ) . '</button>
                    <button class="cookie-btn decline" data-tf-cookies-btn>' . esc_html( $cookies_decline_btn ) . '</button>
                </div>
            </div>
        ';
...
function tf_cat_search_form() {
    ...
    <form method="get" action="<?php print esc_url( home_url( '/' ) );?>">
        <div>
            ...
            <input type="search" name="s" placeholder="<?php print esc_attr__( 'Keyword here...', 'vistro-core' );?>">
            <?php $terms = ( !empty( get_terms( 'category' ) ) ) ? get_terms( 'category' ) : '';?>
            <select>
                <option selected disabled><?php print esc_html__( 'Categories', 'vistro-core' );?></option>
